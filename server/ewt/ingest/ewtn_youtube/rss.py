...
class EWTNYoutubeFeedingService(RSSFeedingService):
    ...
    def _get_xml_item(self, data):
        return next(
            (
                xml_item
                for xml_item in self.tree.findall("entry", NSMAP)
    ...
    def _create_item(self, data, field_aliases=None, source="source"):
        ...
        item["abstract"] = self._substring_before("————", item["abstract"]).strip()
        item["abstract"] = self._substring_before(
            "Subscribe to EWTN’s YouTube Channel", item["abstract"]
        ).strip()
        ...
        item["subject"] = [{"name": "Video", "qcode": "video", "scheme": "atype"}]
        xml_item = self._get_xml_item(data)
        yt_video_link = (
            xml_item.find("media:group", NSMAP)
            .find("media:content", NSMAP)
            .attrib["url"]
        )
        item["body_html"] = item["body_html"] + f"<p>Video link: {yt_video_link}</p>"
        media_title = (
            xml_item.find("media:group", NSMAP).find("media:title", NSMAP).text.strip()
        )
