...
def set_default_template_metadata(item, **kwargs):
    ...
    vocabularies = get_resource_service("vocabularies").get(
        req=None, lookup={"field_type": {"$exists": True}}
    )
    ...
    for key, value in data.items():
        if (
            not item.get(key) and key not in fields_to_exclude
        ) or key in fields_to_override:
    ...
    for sub in subject:
        if sub.get("scheme") and not any(
            subjects.get("scheme") == sub.get("scheme")
            for subjects in item.get("subject", [])
        ):
            to_add.append(sub)

    # and now we add the new one
    item.setdefault("subject", []).extend(
        [i for i in to_add if i.get("scheme") and to_add]
    )
...
label = "Set Default Template Metadata"
